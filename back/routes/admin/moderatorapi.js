...
router.get('/', (req, res, next) => {
    knex.select('uid', 'showname', 'users.ucid', 'ucname').from('users').join('userclasses', 'users.ucid', 'userclasses.ucid').whereNot('uid', '=', res.locals.auth.uid).orderBy('showname', 'ASC')
...
router.patch('/:id', (req, res) => {
    ...
    knex('users').where('uid', '=', req.params.id).update(updData)
